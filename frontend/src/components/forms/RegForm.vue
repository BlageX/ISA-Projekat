          ...
          <div class="card-body">
            ...
            <div v-if='!this.submitted'>
              <form class="form-signin">
                ...
                <div class="form-label-group">
                  <input v-model="form.grad" type="text" id="inputCity" class="form-control" placeholder="Insert city" required autofocus>
                  <label for="inputCity">Grad</label>
                </div>

                <div class="form-label-group">
                  <input v-model="form.drzava" type="text" id="inputCountry" class="form-control" placeholder="Insert country" required autofocus>
                  <label for="inputCountry">Drzava</label>
                </div>

                <div class="form-label-group">
                  <input v-model="form.brojTelefona" type="text" id="inputPhoneNumber" class="form-control" placeholder="Insert phone number" required autofocus>
                  <label for="inputPhoneNumber">Broj telefona</label>
                </div>

                <div class="form-label-group">
                  <input v-model="form.brojOsiguranika" type="text" id="inputNoOs" class="form-control" placeholder="Insert name" required autofocus>
                  <label for="inputNoOs">Broj osiguranika</label>
                </div>
                ...
                <div class="form-label-group">
                  ...
                  <label for="inputUsername">Korsinicko ime</label>
                </div>
                ...
                <button type="button" class="btn btn-lg btn-primary btn-block text-uppercase" v-on:click='submition'>Register</button>
  ...
  methods: {
    submition:function() {
      if(this.form.password ===  this.confPassword){
        PacijentDataService.registratingPacijent({
          ...
          brojTelefona:this.form.brojTelefona,
          email:this.form.email,
          brojOsiguranika:this.form.brojOsiguranika
        }).then(response =>{
          ...
          if(error.response.status === 500  && error.response.data.message==='Username already exists you mappet!'){
            this.messageConfirmPassword = `<h5>Vec postoji korisnik sa unetim <b>korisnickim imenom<b> i <b>brojem osiguranika<b> ! Molimo Vas pokusajte uneti druge vrednosti !</h5>`;
